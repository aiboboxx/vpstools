...
async function freeokSign(row, page) {
  ...
  if ((dayjs.tz().unix() -  dayjs.tz(row.reset_time?row.reset_time:"2006-07-02 08:09:04").unix()) / (24 * 60 * 60) > 10) {
...
async function main() {
  ...
  let sql = "SELECT id,usr,pwd,cookies,regtime,reset_time FROM freeok where  site = 'bjd' and err=1 order by fetch_time asc;"
