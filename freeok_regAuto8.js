...
/*   await page.waitForSelector('#embed-captcha > div');
  ...
  let sql, arr;
  sql = 'insert into  freeok (usr,pwd,cookies,level,reset_time,site) values (?,?,?,8,now(),"okgg");';
  arr = [usr, pwd, ck];
...
async function main() {
  let sql = "SELECT count(*) AS Number FROM freeok where site = 'okgg' and err is null and level = 8 and count < 3;"
  let r = await pool.query(sql);
  if ( r[0][0].Number >= 3 ) {
    console.log('已有2个level=8空闲账户');
    return;
  }
