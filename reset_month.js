...
async function getCount() {
  const result = await pool.query('SELECT count(*) AS Number from paylog WHERE type = 5 and regtime > date_sub(now(), interval 30 day) and fid = '+row.id)
...
async function getDelCount() {
  const result = await pool.query('delete from paylog WHERE type = 5 and regtime < date_sub(now(), interval 30 day) and fid = '+row.id)
  //console.log(JSON.stringify(result))
