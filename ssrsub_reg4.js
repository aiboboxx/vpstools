...
async function regFreeok(page,invite){
  ...
  await sleep(3000);
  let sql, arr;
  sql = 'insert into  freeok (usr,pwd,reset_time,site,level) values (?,?,NOW(),"ssrsub",4);';
...
async function main() {
  ...
  let sql = "SELECT count(*) AS Number FROM freeok where site = 'ssrsub' and level = 4 and used_num < 35;"
  let r = await pool.query(sql);
  //console.log(JSON.stringify(r))
  if ( r[0][0].Number >= 2 ) {
    console.log('已有2以上个level=4 ssrsub账户',r[0][0].Number);
    ignoreA = true;
  }
