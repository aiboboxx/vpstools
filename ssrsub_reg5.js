...
async function regFreeok(page,invite){
  ...
  await sleep(300);
  let sql, arr;
  sql = 'insert into  freeok (usr,pwd,reset_time,site,level) values (?,?,NOW(),"ssrsub",5);';
...
async function main() {
  ...
  let sql = "SELECT count(*) AS Number FROM freeok where site = 'ssrsub' and level = 5 and used_num < 35;"
  let r = await pool.query(sql);
  //console.log(JSON.stringify(r))
  if ( r[0][0].Number >= 4 ) {
    console.log('已有4以上个level=5 ssrsub账户',r[0][0].Number);
    ignoreA = true;
  }
