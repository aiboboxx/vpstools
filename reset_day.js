...
async function getCount() {
  const result = await pool.query('SELECT count(*) AS Number from paylog WHERE type = 4 and regtime > date_sub(now(), interval 1 day)')
...
async function getDelCount() {
  const result = await pool.query('delete from paylog WHERE type = 4 and regtime < date_sub(now(), interval 1 day)')
  //console.log(JSON.stringify(result))
