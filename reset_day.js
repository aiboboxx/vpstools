...
async function getCount() {
  const result = await pool.query('SELECT count(*) AS Number from paylog WHERE type = 4 and regtime > date_sub(now(), interval 1 day) and id = '+row.id)
...
async function getDelCount() {
  const result = await pool.query('delete from paylog WHERE type = 4 and regtime < date_sub(now(), interval 1 day) and id = '+row.id)
  //console.log(JSON.stringify(result))
