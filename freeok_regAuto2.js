...
/*   await page.waitForSelector('#embed-captcha > div');
  ...
  let sql, arr;
  sql = 'insert into  freeok (usr,pwd,cookies,level,reset_time) values (?,?,?,2,now());';
  arr = [usr, pwd, ck];
...
async function main() {
  let sql = "SELECT count(*) AS Number FROM freeok where level = 2 and count < 5;"
  let r = await pool.query(sql);
  if ( r[0][0].Number >= 2 ) {
    console.log('已有3个level=2空闲账户');
    return;
  }
