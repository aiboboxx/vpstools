...
/*   await page.waitForSelector('#embed-captcha > div');
  ...
  let sql, arr;
  sql = 'insert into  freeok (usr,pwd,cookies,level,reset_time,site) values (?,?,?,5,NOW(),"okgg");';
  arr = [usr, pwd, ck];
...
async function main() {
  let sql = "SELECT count(*) AS Number FROM freeok where site = 'okgg' and level = 5 and count < 8;"
