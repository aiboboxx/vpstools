...
/*   await page.waitForSelector('#embed-captcha > div');
  ...
  let sql, arr;
  sql = 'insert into  freeok (usr,pwd,cookies,level,reset_time) values (?,?,?,3,now());';
  arr = [usr, pwd, ck];
...
async function main() {
  let sql = "SELECT id FROM freeok where level = 3 and count < 2;"
  let r = await pool.query(sql);
  if ( r[0].length > 2 ) {
    console.log('已有3个level=3空闲账户');
    return;
  }
