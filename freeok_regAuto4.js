...
/*   await page.waitForSelector('#embed-captcha > div');
  ...
  let sql, arr;
  sql = 'insert into  freeok (usr,pwd,cookies,level,reset_time,site) values (?,?,?,4,NOW(),"okgg");';
  arr = [usr, pwd, ck];
...
async function main() {
  let sql = "SELECT count(*) AS Number FROM freeok where site = 'okgg' and level = 4 and count < 4;"
  sql = "SELECT count(*) AS Number FROM freeok where site = 'okgg' and level = 1;"
  let r = await pool.query(sql);
  if ( r[0][0].Number >= 1 ) {
    console.log('已有1个level=4空闲账户',r[0][0].Number);
    return;
  }
