...
/*   let selecter = 'body > div.geetest_fullpage_click.geetest_float.geetest_wind.geetest_slide3 > div.geetest_fullpage_click_wrap > div.geetest_fullpage_click_box > div > div.geetest_wrap > div.geetest_widget > div > a > div.geetest_canvas_img.geetest_absolute > div > canvas.geetest_canvas_bg.geetest_absolute';
  ...
  async function btnSlider(distance) {
    ...
    await page.mouse.move(box.x + distance / 4 + getRndInteger(-18, 18), axleY + getRndInteger(-18, 18), { steps: getRndInteger(10, 20) });
    await sleep (getRndInteger(50, 200));
    await page.mouse.move(box.x + distance / 2 + getRndInteger(-18, 20), axleY + getRndInteger(-18, 20), { steps: getRndInteger(5, 15) });
    await sleep (getRndInteger(50, 200));
    await page.mouse.move(box.x + (distance / 8) * 7 + getRndInteger(-18, 18), axleY + getRndInteger(-18, 18), { steps: getRndInteger(6, 15) });
    await sleep (getRndInteger(50, 200));
    await page.mouse.move(box.x + distance + getRndInteger(-18, 20), axleY + getRndInteger(-18, 20), { steps: getRndInteger(8, 15) });
    await sleep (getRndInteger(50, 200));
